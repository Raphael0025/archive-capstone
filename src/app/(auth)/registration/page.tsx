...
import { firestore, registerUserData, verifyID } from '../../../lib/controller'
...
export default function Registration(){
    ...
    const validateForm = async (e : React.FormEvent<HTMLFormElement>) => {
        ...
        if(Object.keys(newErrors).length === 0){
            try{
                setIsLoading(true)
                const userId = await registerUserData(showPan, {
                    userName,
                    password,
                    fullName,
                    studID,
    ...
    return(
        <main className="w-full h-dvh grid place-items-center p-8 bg-cover login-library">
            <section className={`lg:w-fit md:w-3/4 sm:full p-8 h-fit flex flex-col bg-maroon rounded ${showPan !== '' ? 'hidden' : ''}`}>
            ...
            <section className={`lg:w-1/2 md:w-3/4 sm:full p-8 h-fit flex flex-col bg-maroon rounded ${showPan === '' ? 'hidden' : ''}`}>
                <div className='w-full flex justify-center flex-col '>
