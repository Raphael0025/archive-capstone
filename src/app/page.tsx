...
export default function Home() {
  ...
  useEffect(() => {
    // Fetch top views when the component mounts
          ...
          <div className='flex md:flex-row flex-col pt-2 w-full justify-between'>
            {Array.isArray(topViews) &&
              topViews.map((download) => (
                <Link key={download.id} href={`/publications/${download.slug}`} passHref>
                  <PDFThumbnail data={download} width={'200px'} height={'260px'}  />
                </Link>
              ))}
          </div>
        </div>
        <div className='p-3 px-4 w-full'>
          ...
          <div className='flex flex-col md:flex-row pt-2 w-full justify-between'>
            {currentArticles.map((article) => (
              <Link key={article.id} href={`/publications/${article.slug}`} passHref>
                <PDFThumbnail data={article} width={'200px'} height={'260px'} />
              </Link>
            ))}
          </div>
